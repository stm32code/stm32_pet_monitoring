...
//	}
	...
	while ( result = esp8266_send_cmd( "AT+RST", "OK", 00 ) )           /* 检查WIFI模块是否在线 */
	{
		printf("AT返回：%d \n",result);
		printf( "正在重启!!!" );
		delay_ms( 800 );
	...
	while ( result = esp8266_send_cmd( "AT", "OK", 200 ) )           /* 检查WIFI模块是否在线 */
	{
		printf("AT返回：%d \n",result);
	...
	delay_ms( 10 );
  printf("准备连接wifi\n");
	while ( esp8266_send_cmd( "AT+CWJAP=\"963\",\"linxy963369\"", "OK", 3000 ) )   /* 连接wifi */
	{
		printf("正在连接wifi，请稍后。。。\n");
		delay_ms( 1000 );
	}
  printf("连接wifi成功 \n");
	...
	while ( esp8266_send_cmd( "AT+CIPSTART=\"TCP\",\"longqiang.top\",8221", "OK", 3000 ) )
	{
