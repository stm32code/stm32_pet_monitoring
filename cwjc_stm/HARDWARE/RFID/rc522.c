...
//	char data;
    ...
    delay_ms( 1000 );		
		sprintf( (char *) p, "GET http://192.168.43.109:9999/user/puttRfid?rfidId=%s", num );      
	  ...
	  u2_printf( "%s\r\n", p );
		...
		strIntercept( USART2_RX_BUF, "{", "}", cRes );
		printf("111 %s \r\n",USART2_RX_BUF);
		sprintf( (char *) p, "{%s}", cRes );
		
		printf("%s \r\n",p);

//		parseJson( p );
		parseJson( USART2_RX_BUF );
		printf("msg:%d,g1:%d,g2:%d,g3:%d,g4:%d\n\r",msg,g1,g2,g3,g4);
		delay_ms( 1000 );
		if(msg == 1){
			...
			u2_printf( "GET http://192.168.43.109:9999/user/book/rfid/startGetRfid\r\n", p );
			delay_ms( 1000 );
		delay_ms( 1000 );
			
			sprintf( (char *) p, "GET http://192.168.43.109:9999/user/updateStatus?rfidId=%s", num );      
